...
int main()
{
	...
	printf("\n      **************             编译原理实验             ******************\n");
	printf("\n      ****  班别：17信息安全2班  姓名：吴泓川  学号：3117005288         ****\n\n");
	printf("      *                   开始调试代码时间：6月23号                        *  \n");
	printf("      *                   完成调试代码时间：6月30号                        *  \n");
	printf("      *                                                                    *  \n");
	printf("\n      **********************************************************************\n");
...
void error(int n)
{
	...
	space[cc - 1] = 0;   //错误时当前符号已经读完，所以cc-1
...
int test(bool* s1, bool* s2, int n)
{
	...
	{
		error(n);
		/*当检测不通过时，不停获取符号，直到它属于需要的集合或者补救的集合*/
...
int block(int lev, int tx, bool* fsys)
{
	...
	do {
		...
		if (sym == constsym)    /* 收到常量声明符号，开始处理常量声明 */
		{
			...
			do {
				constdeclarationdo(&tx, lev, &dx);  /* dx的值会被constdeclaration 改变，使用指针 */
				while (sym == comma)
				{
					getsymdo;
				...
				if (sym == callsym) /* 准备按照call语句处理 */ 
				{
					...
					else
					{
						...
						else
						{
							...
							{
								gendo(cal, lev - table[i].level, table[i].adr);   /* 锟斤拷锟斤拷call指锟斤拷 */
							}
							else
							{
...
int factor(bool* fsys, int* ptx, int lev)
{
	...
	while (inset(sym, facbegsys))    /*循环直到不是因子开始符号*/
	{
		if (sym == ident)            /*因子为常量或变量*/
		{
			...
			getsymdo;
			if (sym == dplus) {
				printf("run to here --- factor-sym==dplus \n");
				gendo(lit, table[i].level, 1);
				gendo(opr, table[i].level, 2);
				gendo(sto, lev - table[i].level, table[i].adr);
				gendo(lod, lev - table[i].level, table[i].adr);
				gendo(lit, 0, 1);
				gendo(opr, 0, 3);
				...
				printf("run to here --- factor-sym==dminus \n");
				gendo(lit, table[i].level, 1);
				gendo(opr, table[i].level, 3);
				gendo(sto, lev - table[i].level, table[i].adr);
				gendo(lod, lev - table[i].level, table[i].adr);
				gendo(lit, 0, 1);
				gendo(opr, 0, 2);
		...
		else
		{
			...
			else
			{
				...
				{
					printf("run to here --- 因子表达式factor-sym==dplus \n");
					getsymdo;
					if (sym == ident) {
						getsymdo;
						i = position(id, *ptx);
				...
				{
					printf("run to here --- 因子表达式factor-sym==dminus \n");
					getsymdo;
					if (sym == ident) {
						getsymdo;
						i = position(id, *ptx);
