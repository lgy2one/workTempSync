...
/*
//漏掉空格，读取一个字符。
//每次读一行，存入line缓冲区，line被getsym取空后再读一行被函数getsym调用。
*/
int getch()
{
	if(cc==ll)
...
//测试当前符号是否合法
//
//在某一部分（如一条语句，一个表达式）将要结束时，
//我们希望下一个符号属于某集合（该部分的后跟符号），
//test负责这项检测，
//并且负责当检测不通过时的补救措施，
//程序在需要检测是指定当前需要的符号集合和补救用的集合（如之前未完成部分的后跟符号），
//以及检测不通过时的错误号
//
//s1:我们需要的符号
