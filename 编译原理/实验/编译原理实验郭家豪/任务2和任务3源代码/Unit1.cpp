...
typedef enum  { NUL, IDENT, NUMBER, PLUS, MINUS, TIMES,
	            ...
	            CONSTSYM, VARSYM, PROCSYM, PROGSYM, ELSESYM,
                    FORSYM, TOSYM, DOWNTOSYM, RETURNSYM, TIMESBECOMES,
                    SLASHBECOMES, PLUSONE, MINUSONE, ANDSYM, ORSYM, NOTSYM
        } SYMBOL;
char *SYMOUT[] = {"NUL", "IDENT", "NUMBER", "PLUS", "MINUS", "TIMES",
	    ...
	    "CONSTSYM", "VARSYM", "PROCSYM", "PROGSYM", "ELSESYM", "FORSYM",
            "TOSYM", "DOWNTOSYM", "RETURNSYM", "TIMESBECOMES", "SLASHBECOMES",
            "PLUSONE", "MINUSONE", "ANDSYM", "ORSYM", "NOTSYM" };
...
void STATEMENT(SYMSET FSYS,int LEV,int &TX) {   /*STATEMENT*/
  ...
  switch (SYM) {
	...
	case CALLSYM:
		...
		break;
	case IFSYM:
		GetSym();                                                       /*判断为IF后继续判断后面是否为THEN的语法分析*/
		CONDITION(SymSetUnion(SymSetNew(THENSYM,DOSYM),FSYS),LEV,TX);   /*对IF判断条件生成相应的目标代码*/
		if (SYM==THENSYM) GetSym();
		else Error(16);
		CX1=CX;  GEN(JPC,0,0);                                          /*假设我的判断条件目标代码最后一句在6地址*/
		/*STATEMENT(FSYS,LEV,TX);  CODE[CX1].A=CX;*/                    /*那么此时的CX就是7地址，而JPC,0,0的代码的地址就是
                                                                                这个7地址。等到STATEMENT生成对应操作的目标代码后
                                                                                CX的地址会再次变化。现在把CX1原来7地址的目标代码
                                                                                A位置处的地址改成此时CX的地址，当逻辑假的时候，就会执行
                                                                                JPC功能，跳到STATEMENT目标代码后，实现逻辑加功能。
                                                                                而现在因为要扩展，这条语句被注释了。*/
                /*以下为ELSE功能扩展代码添加*/
                STATEMENT(SymSetUnion(SymSetNew(ELSESYM),FSYS),LEV,TX);         /*同上，生成目标代码，此时对SYM进行更新判断
                                                                                判断后续符号集合，在FSYS符号集合上增加ELSESYM*/
                if(SYM!=ELSESYM) CODE[CX1].A=CX;                                /*如果SYM不是ELSE，也就是IF THEN后面没有ELSE语句
                                                                                那么就按照上面所说的一样把CX1地址给JPC的A位置就可以了,
                                                                                那么当逻辑假，就可以跳过要执行的目标代码*/
                else{                                                           /*如果后面是ELSE，就存在逻辑假的操作目标代码。*/
                        GetSym();
                        CX2=CX;                                                 /*CX2是第二条功能语句无条件跳转JMP的地址，跳过ELSE操作的目标代码*/
                        GEN(JMP,0,0);                                           /*如果逻辑真，在执行完逻辑真的目标代码后，没有必要继续执行
                                                                                ELSE的目标代码，就可以执行这条无条件跳转目标代码*/
                        CODE[CX1].A=CX;                                         /*相应的，原来在CX1地址的JPC目标代码的目标地址也修改为现在的CX*/
                        STATEMENT(FSYS,LEV,TX);                                 /*生成ELSE操作的目标代码*/
                        CODE[CX2].A=CX;                                         /*ELSE操作的目标代码后的地址，给予JMP无条件跳转*/
                }
		break;
	case BEGINSYM:
		GetSym();
