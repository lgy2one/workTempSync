...
//---------------------------------------------------------------------------     //实验一:增加单词：保留字 ELSE，FOR，TO，DOWNTO，RETURN
                                                                                  //运算符 +=，-=，++，--
                                                                                  //首先是增加单词，修改typedef enum{}SYMBOL,增加相应的变量
                                                                                  //接着修改char *SYMOUT[],同样也增加相应的单词
                                                                                  //现在保留字一共有14+5=19个了，所以修改NORW=19
#include <vcl.h>                                                                  //修改ButtonRunClick函数中的strcpy(KWORD[..],"")及WSYM[..];
#pragma hdrstop                                                                   //修改typedef enum{}SYMBOL,增加四个运算符的标号
#include "Unit1.h"                                                                //修改char *SYMOUT[]，增加四个运算符对应的名称
//---------------------------------------------------------------------------
...
typedef enum  { NUL, IDENT, NUMBER, PLUS, MINUS, TIMES,
	            ...
	            CONSTSYM, VARSYM, PROCSYM, PROGSYM ,ELSESYM,FORSYM,
                    TOSYM,DOWNTOSYM,RETURNSYM,PLUSEQ,PLUSP, MINUSEQ,MINUSM
        } SYMBOL;
char *SYMOUT[] = {"NUL", "IDENT", "NUMBER", "PLUS","PLUSEQ","PLUSP", "MINUS",
                  "MINUSEQ","MINUSM", "TIMES",
	    "SLASH", "ODDSYM", "EQL", "NEQ", "LSS", "LEQ", "GTR", "GEQ",
	    "LPAREN", "RPAREN", "COMMA", "SEMICOLON", "PERIOD",
	    "BECOMES", "BEGINSYM", "ENDSYM", "IFSYM", "THENSYM",
	    "WHILESYM", "WRITESYM", "READSYM", "DOSYM", "CALLSYM",
...
//---------------------------------------------------------------------------
void GetCh() {                                                                     //int    CC;  /*CHARACTER COUNT*/
  if (CC==LL) {                                                                    //int    LL;  /*LINE LENGTH*/
    if (feof(FIN)) {
	...
	LL=0; CC=0;
	CH=' ';
	while (!feof(FIN) && CH!=10)     //10为换行
      { CH=fgetc(FIN);  LINE[LL++]=CH; }
	LINE[LL-1]=' ';  LINE[LL]=0;                                              //char   LINE[81];
    String s=IntToStr(CX);                                                        //int    CX;  /*CODE ALLOCATION INDEX*/
...
//---------------------------------------------------------------------------
void GetSym() {                                                                    //GetSym()函数，获取单词，并存入到ID当中
  int i,J,K;   ALFA  A;                                                            //typedef  char ALFA[11];
  while (CH<=' ') GetCh();                                                         //char   CH;  /*LAST CHAR READ*/
  if (CH>='A' && CH<='Z') { /*ID OR RESERVED WORD*/
    K=0;
	do {
	  if (K<AL) A[K++]=CH;
	  GetCh();                                                                //const  AL    =  10;  /* LENGTH OF IDENTIFIERS */
	}while((CH>='A' && CH<='Z')||(CH>='0' && CH<='9'));
	A[K]='\0';
	strcpy(ID,A); i=1; J=NORW;                                                 //ALFA   ID;  /*LAST IDENTIFIER READ*/
	do {                                                                        //const  NORW  =  14;  /* # OF RESERVED WORDS */
	  K=(i+J) / 2;
	  if (strcmp(ID,KWORD[K])<=0) J=K-1;                                      //ALFA    KWORD[NORW+1];
	  if (strcmp(ID,KWORD[K])>=0) i=K+1;
	}while(i<=J);
	if (i-1 > J) SYM=WSYM[K];                                                  //WSYM 存放保留字的类型,一共有14个保留字，所以共有14个类型
	else SYM=IDENT; //说明不是保留字,则是变量名,即标识符
  }
  else
    if (CH>='0' && CH<='9') { /*NUMBER*/
	  ...
	  do {
	    NUM=10*NUM+(CH-'0');
		K++; GetCh();                                                      //若所读取的是数字，则SYM=NUMBER，并且值保存到NUM中。
      }while(CH>='0' && CH<='9');
	  if (K>NMAX) Error(30);
    }
    else
      if (CH==':') {
	    GetCh();
		if (CH=='=') { SYM=BECOMES; GetCh(); }                             //当读到:=时，SYM=BECOMES
		else SYM=NUL;
      }
      else if(CH=='#')
      {
	    ...
	    if (CH=='<') {
		  ...
		  if (CH=='=') { SYM=LEQ; GetCh(); }                              //遇到<=时，SYM=LEQ
		  else SYM=LSS;                                                   //遇到<时，SYM=LSS
		}
		else
		  if (CH=='>') {
		    GetCh();
			if (CH=='=') { SYM=GEQ; GetCh(); }                       //遇到>=时，SYM=GEQ
			else SYM=GTR;                                            //遇到>时，SYM=GTR
          }
		  else { SYM=SSYM[CH]; GetCh(); }                                //其它情况则为+-*/().;等等
} /*GetSym()*/
//---------------------------------------------------------------------------
  ...
  switch (SYM) {
	...
	case IFSYM:
		...
		CONDITION(SymSetUnion(SymSetNew(THENSYM),FSYS),LEV,TX);
                ...
                if(SYM != ELSESYM)
                        CODE[CX1].A = CX;//只有if的情况，这里直接回填
                else
                {
                        CX2 = CX;//由于有else部分，供jmp跳转回填地址
                        GetSym();
                        GEN(JMP,0,0);
                        CODE[CX1].A = CX;//回填if判断跳转地址
                        STATEMENT(FSYS,LEV,TX);
                        CODE[CX2].A = CX; //回填jmp跳转地址
                }
                break;
...
void Block(int LEV, int TX, SYMSET FSYS) {
  ...
  do {
    ...
    TEST(SymSetAdd(IDENT,STATBEGSYS), DECLBEGSYS,7);//测试过程开始语句是否正确
...
void __fastcall TForm1::ButtonRunClick(TObject *Sender) {
  ...
  DECLBEGSYS=(int*)malloc(sizeof(int)*33);                                        //这里的大小为33个是因为前面定义的SYMBOL枚举量大小为33
  STATBEGSYS=(int*)malloc(sizeof(int)*33);                                        //以后进行扩充时需要增加
